...
import report_404  # report_404에서 stock 클래스 사용한다
...
def portfolio_cost(filename):
    ...
    :return 보유종목 현재값 합계
    """
    portfolio = report_404.read_portfolio(filename)  # 포트폴리오 파일을 읽어서 인스턴스 리스트로 만든다
    return sum([s.cost() for s in portfolio])  # 포트폴리오 각 인스턴스에 대해서 "수량*가격"을 모두 더한다
